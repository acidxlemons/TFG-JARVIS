...
En el \textbf{Capítulo~\ref{cap:estado}} se expone el estado del arte de los LLMs, los sistemas RAG, los modelos de embeddings, las bases de datos vectoriales, técnicas de cuantización y MCP.

El \textbf{Capítulo~\ref{cap:metodologia}} describe la metodología seguida, las iteraciones del proyecto, el entorno hardware, el stack tecnológico completo y las herramientas de gestión y documentación utilizadas.

El \textbf{Capítulo~\ref{cap:arquitectura}} profundiza en el diseño de la arquitectura general del sistema, detallando sus capas funcionales, la orquestación con Docker, el flujo de datos extremo a extremo y el stack de observabilidad.

El \textbf{Capítulo~\ref{cap:resultados}} explica el proceso de implementación de cada subsistema principal (JARVIS, embeddings, web scraping, OCR, SharePoint, BOE y fine-tuning), acompañado de fragmentos de código, métricas de rendimiento y una evaluación cualitativa de resultados.

Por último, el \textbf{Capítulo~\ref{cap:conclusiones}} recoge las conclusiones, lecciones aprendidas y futuras líneas de trabajo.