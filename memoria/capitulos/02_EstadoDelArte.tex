...
En este proyecto se utiliza un modelo multilingüe de SentenceTransformers por compatibilidad con documentación en español/inglés y facilidad de despliegue local \cite{sbertDocs}. Adicionalmente, el sistema contempla un proceso de \textit{fine-tuning} sobre este modelo base para mejorar la representación semántica en el dominio específico corporativo, ajustando el espacio vectorial a la terminología propia.
...
\label{sec:lora}
% ============================================================
El ajuste fino (\textit{fine-tuning}) adapta un modelo generalista a un dominio concreto enseñándole nueva terminología, estilo o formatos de respuesta. Sin embargo, entrenar todos los parámetros de un LLM moderno requiere una capacidad de cómputo inmensa.

Para resolver este problema, la técnica LoRA (\textit{Low-Rank Adaptation}) propone entrenar únicamente unos adaptadores de bajo rango que se insertan en las capas de atención del modelo original. Durante el entrenamiento, los pesos originales se mantienen congelados (\textit{frozen}) y solo se actualizan los adaptadores, reduciendo drásticamente el número de parámetros a entrenar y haciendo viable el ajuste fino con GPUs moderadas \cite{hu2021lora}:
...
En este proyecto se explora el ajuste fino mediante LoRA sobre un LLM base con el objetivo de mejorar su adherencia a un formato específico. No obstante, se observó empíricamente que, para tareas de RAG puras, el \textit{fine-tuning} en el LLM resultaba menos eficaz que un buen ajuste del \textit{prompt} (\textit{prompt engineering}) combinado con un modelo de embeddings bien adaptado al dominio. Por ello, la estrategia final descansa más sobre la calidad del contexto recuperado que sobre la modificación de los pesos del LLM.
