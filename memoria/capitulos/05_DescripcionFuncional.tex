...
En este capítulo describo el sistema que he desarrollado desde la perspectiva del usuario final. Detallo qué puede hacer el sistema, cómo se presenta la interfaz conversacional, qué funcionalidades están disponibles y cómo puede interactuar el usuario con cada una de ellas. Mi objetivo es ofrecer una visión completa y comprensible de las capacidades del sistema sin entrar todavía en los detalles de implementación técnica, que reservo para el Capítulo~\ref{cap:implementacion}.
...
He diseñado el sistema para que \textbf{detecte automáticamente la intención del usuario} a partir del texto del mensaje, sin necesidad de comandos explícitos, menús ni modos manuales. El usuario simplemente escribe su pregunta o solicitud y el sistema decide internamente qué funcionalidad activar. Esta transparencia hace que la experiencia sea natural e intuitiva, similar a interactuar con un asistente humano.
...
La interfaz de usuario es una aplicación web tipo chat, similar en aspecto y funcionamiento a ChatGPT o Microsoft Copilot. La interfaz se estructura en los siguientes elementos principales:
...
Las respuestas se muestran en \textbf{streaming en tiempo real}: el texto aparece progresivamente, token a token, a medida que el modelo lo genera. Esto proporciona retroalimentación inmediata al usuario y evita la sensación de espera prolongada.
...
\begin{itemize}
    ...
    \item \textbf{Emojis:} Para indicaciones visuales de modo de operación (📚 RAG, 🌐 Web, 🔍 Búsqueda, 🖼️ Visión).
\end{itemize}
...
A continuación describo los modos de operación fundamentales que he configurado en el sistema, vistos desde la perspectiva del usuario, con ejemplos de los tipos de consultas que activan cada modo.
...
\begin{itemize}
    ...
    \item \textbf{Indicador visual:} 📚 \texttt{Buscando en documentos internos...}
\end{itemize}

Las respuestas en modo RAG incluyen \textbf{referencias a las fuentes}: el nombre del documento, la página y la puntuación de relevancia de cada fragmento utilizado, proporcionando trazabilidad completa.
...
\begin{itemize}
    ...
    \item \textbf{Indicador visual:} 🌐 \texttt{Buscando en internet...}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Indicador visual:} 🔍 \texttt{Analizando URL...}
\end{itemize}

El contenido extraído queda almacenado en la \textbf{memoria de sesión del usuario}, lo que permite realizar preguntas de seguimiento como ``¿Qué dice el tercer párrafo?'' sin necesidad de volver a enviar la URL. Además, el usuario puede optar por indexar el contenido permanentemente en la colección \texttt{webs} de Qdrant para futuras consultas RAG.
...
\begin{itemize}
    ...
    \item \textbf{Comportamiento:} La imagen se envía a un modelo de visión multimodal (Qwen 2.5 VL o LLaVA) que describe, transcribe o analiza el contenido visual.
    \item \textbf{Indicador visual:} 🖼️ \texttt{Analizando imagen...}
\end{itemize}
...
El sistema integra la capacidad de consultar el Boletín Oficial del Estado (BOE) directamente desde la interfaz de chat.

\begin{itemize}
    \item \textbf{Ejemplo de consulta:} ``Busca en el BOE la ley de protección de datos''
    \item \textbf{Comportamiento:} El sistema resuelve semánticamente el nombre coloquial de la ley a su identificador oficial del BOE, consulta la API de Datos Abiertos del BOE y devuelve un resumen estructurado de la ley solicitada.
    \item \textbf{Indicador visual:} 🏛️ \texttt{Consultando el BOE...}
\end{itemize}
...
Para ilustrar la experiencia del usuario de forma integral, describo a continuación tres escenarios de uso representativos de las capacidades que he integrando:
