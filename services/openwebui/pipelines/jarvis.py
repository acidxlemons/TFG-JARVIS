...
class Pipeline:
    ...
    def _detect_intent(self, message: str, body: Dict, messages: List[Dict] = None) -> Dict:
        ...
        list_docs_keywords = ["/listar", "/docs", "/documentos", "/list", "list docs", "list", "docs", "webs", "web"]
        list_docs_pattern = r"(?:listar?|listame|dime|ver|mostrar|cuales|que|list)\s+(?:son\s+|los\s+|las\s+|hay\s+|mis\s+|tus\s+|todos\s+los\s+|todas\s+las\s+)?(docs|documentos|archivos|pdfs|guardado|tengo guardado|webs|paginas|sitios|urls)"
        ...
        # 1. Comando: Ayuda / Capacidades (NUEVO)
        help_keywords = [
            "que puedes hacer", "qué puedes hacer", "ayuda", "help", "funcionalidades", 
            "como funciona", "cómo funciona", "que sabes hacer", "qué sabes hacer",
            "comandos", "manual", "instrucciones", "guía", "guia", "capacidades",
            "para que sirves", "para qué sirves", "qué eres", "que eres", "quien eres", "quién eres"
        ]
        ...
        # 2b. Comando: Listar documentos (comando directo) - MOVED TO TOP
        if message_lower in list_docs_keywords:
            return {"action": "list_docs", "metadata": {}}
        
        # 3. Preguntas naturales sobre documentos (MOVED TO TOP)
        collection_pattern = r"(?:en|de|dentro de|about)\s+([a-zA-Z0-9_\-\u00C0-\u00FF]+)"

        collection_match = re.search(collection_pattern, message_lower)
        
        # Try to match the list_docs pattern which captures the target type
        list_docs_match = re.search(list_docs_pattern, message_lower)
        
        if list_docs_match or message_lower in ["docs", "documentos", "archivos", "webs"]:
            metadata = {}
            ...
            elif list_docs_match:
                target_word = list_docs_match.group(1).lower().strip()
        ...
        # 2a. BOE / Legal — ANTES de RAG para evitar que "qué dice" capture BOE
        ...
        law_text_patterns = [
            ...
            r"(?:texto|contenido)\s+(?:de\s+)?(?:la\s+)?(?:ley\s+)?(ley\s+.+|código\s+.+|estatuto\s+.+|constitución|lsi|lopd|rgpd|lssice)",
            r"(?:qué|que)\s+dice\s+(?:la\s+)?(ley\s+.+|código\s+.+|estatuto\s+.+|constitución|lsi|lopd|rgpd|lssice)",
        ]
        ...
        law_analysis_patterns = [
            r"(?:qué|que)\s+(?:modifica|deroga|cambia)\s+(?:la\s+)?(ley\s+.+|real\s+decreto\s+.+|rd\s+.+)",
            r"(?:qué|que)\s+leyes?\s+modifica\s+(?:la\s+)?(.+)",
            r"referencias\s+(?:de\s+)?(?:la\s+)?(ley\s+.+|real\s+decreto\s+.+)",
            r"análisis\s+(?:jurídico\s+)?(?:de\s+)?(?:la\s+)?(ley\s+.+|real\s+decreto\s+.+)",
        ]
        ...
        # 2b. RAG (Documentos internos) — DESPUÉS de BOE
        if any(kw in message_lower for kw in rag_keywords):
            ...
            coll_regex = r"(?:en|de|sobre)\s+(?:las?|los?|mis?)?\s*(webs?|documentos?|archivos?|local(?:es)?|internus?|paginas?|sitios?)"
            coll_match = re.search(coll_regex, message_lower)
            
            if coll_match:
                found_kw = coll_match.group(1)
                for key, val in coll_map.items():
                    if key in found_kw:
                        metadata["target_collection"] = val
                        break
        ...
        # 4a. High Priority: Sticky Context Check (Before Web Search)
        # Prevent Web Search hijacking if we are deepening a RAG conversation
        if messages and len(messages) > 1:
            last_user_msg_c = messages[-2].get("content", "").lower() if len(messages) >= 2 else ""
            was_rag_c = any(kx in last_user_msg_c for kx in rag_keywords)
            
            # If we were in RAG and user asks for "fechas", "resultados" (which act as web keywords too)
            if was_rag_c:
                # Check if current message is a follow-up
                is_follow_up_c = any(w in message_lower for w in ["y", "dime", "que", "cuales", "fechas", "resultados", "mas", "informacion"])
                if is_follow_up_c:
                     logger.info(f"🛡️ Sticky Context: Forcing RAG over Web Search due to context")
                     return {"action": "rag", "metadata": {}}
        ...
        # 7. Document Codes (Moved up)
        
        # 8. Status
        if any(kw in message_lower for kw in status_keywords):
            return {"action": "check_status", "metadata": {}}
            
        # 9. STICKY MODE (Ambiguous follow-ups)
        # Si no se detectó nada explícito, mirar si estamos en medio de una conversación RAG/Web
        # Relaxed constraints: < 25 words or explicit connectors
        msg_len = len(message.split())
        connectors = ["y", "sobre", "que", "cual", "cuanto", "donde", "cuando", "quien", "si", "siii", "vale", "dime"]
        is_connector = any(message_lower.startswith(x) for x in connectors) or any(f" {x} " in f" {message_lower} " for x in connectors)
        
        if msg_len < 25: # Increased threshold
             is_ambiguous = "?" in message or is_connector
             
             if is_ambiguous and messages and len(messages) > 1:
                # Comprobar historial reciente
                last_assistant_msg = messages[-1].get("content", "") if messages[-1]["role"] == "assistant" else ""
                # Heurística simple: Detectar de qué hablaba el asistente
                
                # Si el asistente dio una respuesta de RAG (suele tener citas o estructura específica, pero aquí asumimos persistencia por defecto)
                # Mejor: Usamos flags internas si pudiéramos, pero sin estado, usamos el contenido del user anterior
                
                # ...O más simple: si las keywords de la PREGUNTA ANTERIOR eran de RAG/Web
                last_user_msg = messages[-2].get("content", "").lower() if len(messages) >= 2 else ""
                
                was_rag = any(kx in last_user_msg for kx in rag_keywords)
                was_web = any(kx in last_user_msg for kx in web_search_keywords)
                
                if was_rag:
                     logger.info(f"💾 Sticky Mode: Reusing RAG intent from previous turn")
                     return {"action": "rag", "metadata": {}}
                if was_web:
                     logger.info(f"💾 Sticky Mode: Reusing WebSearch intent from previous turn")
                     return {"action": "web_search", "metadata": {}}
    ...
    def _scrape_and_summarize(self, url: str, user_message: str, user_email: str, check_rag: bool = False):
        ...
        try:
            ...
            # Warning for very low word counts (likely JavaScript-protected page)
            if word_count < 50:
                yield f"⚠️ **Nota**: Solo se extrajeron {word_count} palabras. Es posible que la página esté protegida por JavaScript o requiera login.\n\n"
            
            summary_prompt = f"Analiza el siguiente contenido web:\nTítulo: {title}\nContenido:\n{analysis_content}\n\nPregunta del usuario: {user_message}\n\nProporciona un análisis útil y estructurado. Responde en el idioma de la pregunta del usuario."
            
            try:
                # System prompt más permisivo para evitar rechazos
                system_prompt = "Eres un asistente útil que analiza contenido web. Tu trabajo es resumir y responder preguntas sobre el contenido proporcionado. Siempre intenta ser útil. No rechaces analizar contenido legítimo de noticias o webs públicas."
                summary = self._call_litellm(summary_prompt, self.valves.TEXT_MODEL, system_prompt)
                yield summary
            except Exception as llm_error:
                yield f"{content[:2000]}...\n"
            
            # Guardar en memoria web para follow-ups y "guarda esta pagina"
            ...
            logger.info(f"Web memory saved for {user_email}: {title}")
    ...
    def _resolve_search_query(self, message: str, chat_history: List[Dict]) -> str:
        """Resuelve la query de búsqueda usando el historial si es necesario."""
        if not chat_history or len(chat_history) < 2:
            return message
            
        # Comprobar si vale la pena reescribir (keywords de seguimiento)
        check_msg = message.lower()
        needs_context = any(w in check_msg for w in ["esto", "eso", "el", "la", "mas", "más", "anterior", "sobre"])
        
        if not needs_context and len(message.split()) > 3:
            return message

        logger.info(f"🧠 Resolviendo contexto para búsqueda: {message}")
        try:
            # Construir contexto reciente (últimos 2 mensajes)
            last_msgs = ""
            for msg in chat_history[-2:]:
                role = "User" if msg.get("role") == "user" else "Assistant"
                content = msg.get("content", "")
                if isinstance(content, list): content = " ".join([c.get("text","") for c in content if isinstance(c,dict)])
                last_msgs += f"{role}: {content}\n"
            
            prompt = f"""Based on the conversation history, rewrite the last user query to be a standalone search query. Keep it concise.
            ...
            resolved = self._call_ollama_direct(prompt, system_prompt="You are a query resolution assistant. Output ONLY the resolved query.")
            resolved = resolved.strip().strip('"')
            if len(resolved) > 5:
                logger.info(f"🧠 Query resuelta: '{message}' -> '{resolved}'")
                return resolved
            return message
        except Exception as e:
    ...
    def pipe(self, user_message: str, model_id: str, messages: List[Dict], body: Dict) -> Union[str, Generator, Iterator]:
        try:
            ...
            if action == "list_docs":
                ...
                # Determinar tenant IDs base
                base_tenants = []
                if self.valves.MULTI_DEPARTMENT_ENABLED and authorized_depts:
                    base_tenants = authorized_depts
                else:
                    base_tenants = [self.valves.DEFAULT_COLLECTION]

                # Asegurar que webs está accesible (salvo restricción explícita futura)
                if "webs" not in base_tenants:
                    base_tenants.append("webs")

                # Aplicar filtro si se solicitó colección específica
                final_tenants = base_tenants
                if target_collection:
                    if target_collection.lower() == "webs":
                        final_tenants = ["webs"]
                    elif target_collection.lower() in ["documents", "documentos"]:
                        final_tenants = [t for t in base_tenants if t != "webs"]
                    else:
                        # Filtrar por nombre (ej: "calidad")
                        matched = [t for t in base_tenants if target_collection.lower() in t.lower()]
                        if matched: final_tenants = matched
                
                headers["X-Tenant-Ids"] = ",".join(final_tenants)
                ...
                # Resolver query con contexto
                search_query = self._resolve_search_query(user_message, messages)
                if search_query != user_message:
                     yield f"🔍 *Buscando: {search_query}*\n\n"

                response = requests.get(f"{self.valves.BACKEND_URL}/web-search", params={"q": search_query}, timeout=30)
                if response.status_code == 200:
                ...
                # Resolver query con contexto (Memoria conversacional)
                rag_query = self._resolve_search_query(user_message, messages)
                if rag_query != user_message:
                     yield f"🔍 *Contexto aplicado: {rag_query}*\n\n"
                
                data = self._call_backend_chat(rag_query, "rag", user_data, messages, metadata=metadata)
                content = data.get("content", "")
                
                if messages and len(messages) > 1:
                     # Re-procesar con historial pero con system prompt ESTRICTO
                     # que impide mezclar temas anteriores con documentos internos
                     ctx = f"Resultados de documentos internos:\n{content}"
                     rag_system_prompt = (
                         "Eres un asistente que responde EXCLUSIVAMENTE basándose en los documentos internos proporcionados. "
                         "REGLAS ESTRICTAS:\n"
                         "1. USA SOLO la información de los 'Resultados de documentos internos' para responder.\n"
                         "2. NO mezcles información de conversaciones anteriores sobre otros temas.\n"
                         "3. Si los documentos no contienen la respuesta, di 'No encontré información sobre esto en los documentos disponibles'.\n"
                         "4. Cita siempre el nombre del documento fuente.\n"
                         "5. NUNCA inventes contenido que no esté en los documentos proporcionados.\n"
                         "6. Responde en el idioma del usuario."
                     )
                     content = self._call_litellm_with_history(user_message, rag_system_prompt, messages, ctx)
                ...
                # 1. Estado del indexer/SharePoint
                try:
                    ...
                    yield "⚠️ Indexer no disponible\n"
                
                # 2. Stats de documentos en Qdrant
                try:
                    stats = requests.get(f"{self.valves.BACKEND_URL}/documents/stats", timeout=5).json()
                    points = stats.get("points_count", "?")
                    yield f"📄 Documentos indexados: **{points}** chunks en Qdrant\n"
                except:
                    yield "⚠️ No pude obtener stats de documentos\n"
                
                # 3. Estado de ingesta
                try:
                    ingestion = requests.get(f"{self.valves.BACKEND_URL}/documents/ingestion-status", timeout=5).json()
                    active = ingestion.get("active", [])
                    if active:
                        yield f"🔄 En proceso: {len(active)} documento(s)\n"
                        for item in active[:3]:
                            yield f"   - {item.get('filename', '?')}: {item.get('status', '?')}\n"
                    else:
                        yield "✅ Sin documentos en cola de procesamiento\n"
                except:
                    pass
                
                # 4. Health del backend
                try:
                    health = requests.get(f"{self.valves.BACKEND_URL}/health", timeout=5).json()
                    components = health.get("components", {})
                    yield f"\n**Componentes:**\n"
                    for comp, status in components.items():
                        icon = "✅" if status in ["connected", "ready", "healthy"] else "⚠️"
                        yield f"  {icon} {comp}: {status}\n"
                except:
                    yield "⚠️ No pude obtener health del backend\n"
    ...
    def _get_help_message(self) -> str:
        """Devuelve un mensaje de ayuda formateado según las capacidades del sistema."""
        return """
👋 **¡Hola! Soy JARVIS, tu asistente inteligente.**

Puedo ayudarte con varias tareas. Aquí tienes una guía rápida de mis capacidades:

### 📚 **1. Consultar Documentos Internos (RAG)**
Busco información en los manuales, políticas y procedimientos de tu empresa.
*   "Busca en tus documentos sobre la política de teletrabajo"
*   "¿Qué dice el procedimiento de compras?"
*   "Dame el resumen del manual de calidad"

### 🌐 **2. Buscar en Internet**
Busco información actualizada en la web.
*   "Busca en internet el precio del Bitcoin"
*   "Noticias sobre inteligencia artificial"
*   "¿Quién ganó el último mundial?"

### ⚖️ **3. Consultar el BOE y Legislación**
Consulto el Boletín Oficial del Estado y leyes españolas en tiempo real.
*   "¿Qué dice el BOE de hoy?"
*   "Dame el artículo 10 de la LOPD"
*   "Busca normativa sobre protección de datos"
*   "¿Qué leyes modifica el Real Decreto 28/2020?"

### 🕷️ **4. Analizar Páginas Web**
Puedo leer, resumir y memorizar páginas web específicas.
*   Pega una URL: `https://es.wikipedia.org/wiki/IA`
*   "Analiza https://ejemplo.com"
*   "Indexa https://docs.python.org con profundidad 2" (Scraping recursivo)

### 🖼️ **5. Visión / OCR**
Puedo ver y leer imágenes.
*   **Adjunta una imagen** y pregunta: "¿Qué dice este documento?" o "Describe esta foto".

### 💬 **6. Otros Comandos**
*   `/listar`: Ver qué documentos tengo indexados.
*   `listar webs`: Ver qué páginas web he guardado.
*   `status` / `¿cómo va?`: Ver estado de subida de archivos.

💡 **Consejo**: Simplemente pídeme lo que necesitas en lenguaje natural. ¡Estoy aquí para ayudar!
"""
