...
class Pipeline:
    ...
    def _detect_intent(self, message: str, body: Dict, messages: List[Dict] = None) -> Dict:
        ...
        # 2a. RAG (Documentos internos) - MOVED HERE BEFORE BOE/WEB
        if any(kw in message_lower for kw in rag_keywords):
            ...
            # Buscar menciones explícitas de colección: "en mis documentos", "en las webs", "en local"
            # Regex flexible para capturar preposiciones + artículos
            ...
            if coll_match:
                ...
                # Map partial match to canonical collection
                for key, val in coll_map.items():
                    if key in found_kw:
                        metadata["target_collection"] = val
                        break
            else:
                 # Check simple string match (fallback)
                 for key, val in coll_map.items():
        ...
        law_text_patterns = [
            ...
            r"(?:texto|contenido)\s+(?:de\s+)?(?:la\s+)?(?:ley\s+)?(ley\s+.+|código\s+.+|estatuto\s+.+|constitución|lsi|lopd|rgpd|lssice)", # Require specific law keywords
            r"(?:qué|que)\s+dice\s+(?:la\s+)?(ley\s+.+|código\s+.+|estatuto\s+.+|constitución|lsi|lopd|rgpd|lssice)", # Require specific law keywords
        ]
        ...
        try:
            ...
            if action == "list_docs":
                ...
                if messages and len(messages) > 1:
                     ...
                     ctx = f"Resultados:\n{content}"
                     content = self._call_litellm_with_history(user_message, "You are a helpful assistant. Use the provided context to answer the user request.", messages, ctx)
